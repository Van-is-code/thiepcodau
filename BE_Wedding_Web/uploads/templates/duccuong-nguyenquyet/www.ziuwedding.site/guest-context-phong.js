(function () {
  ...
  function ensureThankYouPopup() {
    ...
    // Lấy tên/ngày/ảnh từ window.invitationData (đã được React host bơm sẵn trong
    // <head> trước khi script này chạy) — có gì dùng nấy, không có thì giữ nguyên
    // giá trị mặc định của mẫu gốc.
    const invData = window.invitationData || {};
    const groomName = invData.groom_name || 'Chú rể';
    const brideName = invData.bride_name || 'Cô dâu';

    let ceremonyDay = '20';
    let ceremonyMonth = '06';
    let ceremonyYear = '2026';
    if (invData.ceremony_date) {
      const d = new Date(invData.ceremony_date);
      if (!Number.isNaN(d.getTime())) {
        ceremonyDay = String(d.getUTCDate()).padStart(2, '0');
        ceremonyMonth = String(d.getUTCMonth() + 1).padStart(2, '0');
        ceremonyYear = String(d.getUTCFullYear());
      }
    }

    const galleryPhoto = (invData.images || [])
      .filter(function (img) { return img.image_type === 'gallery'; })
      .sort(function (a, b) { return (a.sort_order || 0) - (b.sort_order || 0); })[0];
    const photoUrl = (galleryPhoto && galleryPhoto.image_url) ||
    ...
    overlay.innerHTML = `
      <div class="ty-sheet">
        ...
        <div class="ty-photo">
          ...
          <div class="ty-photo-name">${groomName} &amp; ${brideName}</div>
        </div>
        ...
        <div class="ty-body">
          ...
          <div class="ty-date">
            <div class="ty-date-block">
              ...
              <span class="ty-date-num">${ceremonyDay}</span>
            </div>
            <div class="ty-date-sep"></div>
            <div class="ty-date-block">
              <span class="ty-date-label">Tháng</span>
              <span class="ty-date-num">${ceremonyMonth}</span>
            </div>
            <div class="ty-date-sep"></div>
            <div class="ty-date-block">
              <span class="ty-date-label">Năm</span>
              <span class="ty-date-num">${ceremonyYear}</span>
            </div>
          </div>
